locals {
  ...
  database_application_user = "wikione_app"
  ...
  service_definitions = {
    ...
    api = {
      ...
      environment = {
        ...
        API_PORT                    = "3000"
        COOKIE_SECURE               = "true"
        DATABASE_CREDENTIAL_VERSION = tostring(var.database_application_secret_version)
        DATABASE_HOST               = aws_db_instance.postgres.address
        DATABASE_NAME               = aws_db_instance.postgres.db_name
        DATABASE_PORT               = tostring(aws_db_instance.postgres.port)
        DATABASE_USER               = local.database_application_user
      ...
      secrets = merge(var.api_secret_arns, {
        DATABASE_PASSWORD = "${aws_secretsmanager_secret.database_app.arn}:password::"
      })
    }
    preview = {
